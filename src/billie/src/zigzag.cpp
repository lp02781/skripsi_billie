...
int main(int argc, char **argv)
{
	...
	while(ros::ok())
	{
		...
		if(velocity_ctrl == 1)
		{
			...
			rate.sleep();
			fprintf(rpy, "%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f \n",position_reff_x, current_position_x, position_reff_y, current_position_y, current_error_x, current_error_y, current_yaw_reff, current_yaw, mse_x, mse_y, roll, pitch);
		}
...
void position_trajectory(const ros::TimerEvent& event)
{
	...
	{	
		...
		*/
		if(gain_constant >= 0 && gain_constant < duration - (duration*3/4)) 
		{
			position_reff_x = x_awal + (gain_constant/duration)*x_cmd;	
			position_reff_y = y_awal + (gain_constant/(duration/4))*y_cmd;	
		...
		if(gain_constant >= (duration - duration*3/4) && gain_constant < duration - (duration*2/4)) 
		{
			position_reff_x = x_awal + (gain_constant/duration)*x_cmd;	
			position_reff_y = y_awal + y_cmd - ((gain_constant-duration*1/4)/(duration/4))*y_cmd;	
			current_yaw_reff = atan2(position_reff_y-position_reff_y_prv,position_reff_x-position_reff_x_prv)/pi*180;		
			position_reff_x_prv = position_reff_x;
			position_reff_y_prv = position_reff_y;
	
		}
		if(gain_constant >= (duration - duration*2/4) && gain_constant < duration - (duration*1/4)) 
		{
			position_reff_x = x_awal + (gain_constant/duration)*x_cmd;	
			position_reff_y = y_awal + ((gain_constant-duration*2/4)/(duration/4))*y_cmd;	
			current_yaw_reff = atan2(position_reff_y-position_reff_y_prv,position_reff_x-position_reff_x_prv)/pi*180;		
			position_reff_x_prv = position_reff_x;
			position_reff_y_prv = position_reff_y;	
		}
		if(gain_constant >= (duration - duration*1/4) && gain_constant < duration - (duration*0/4)) 
		{
			position_reff_x = x_awal + (gain_constant/duration)*x_cmd;	
			position_reff_y = y_awal + y_cmd - ((gain_constant-duration*3/4)/(duration/4))*y_cmd;	
			current_yaw_reff = atan2(position_reff_y-position_reff_y_prv,position_reff_x-position_reff_x_prv)/pi*180;		
			position_reff_x_prv = position_reff_x;
			position_reff_y_prv = position_reff_y;
		}
	...
	if(pid_ctrl == 1){
		...
		position_reff_y_prv = position_reff_y;
		current_yaw = yaw;
		current_error_yaw = current_yaw_reff - current_yaw;
		...
		ROS_INFO_STREAM("Nilai posisi yaw sekarang adalah " << current_yaw);
		ROS_INFO_STREAM("Nilai posisi yaw reference adalah " << current_yaw_reff);
