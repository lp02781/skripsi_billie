...
int main(int argc, char **argv)
{
	...
	while(ros::ok())
	{
		...
		if(velocity_ctrl == 1)
		{
			...
			rate.sleep();
			fprintf(rpy, "%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f \n",position_reff_x, current_position_x, position_reff_y, current_position_y, current_error_x, current_error_y, yaw_reff, current_yaw, mse_x, mse_y, roll, pitch);
		}
...
void position_trajectory(const ros::TimerEvent& event)
{
	...
	{	
		...
		*/
		...
		{
			position_reff_x = x_awal + gain_constant;	
			position_reff_y = y_awal + y_cmd*sin((gain_constant/duration)*2*pi);	
...
//		velocity.twist.angular.z = dv;
		...
		ROS_INFO_STREAM("Nilai Yaw reference adalah " << current_yaw_reff);
