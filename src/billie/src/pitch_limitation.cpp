...
//				usleep(1000000);
			...
			g_key_command = 'x';
			cmd_count = 1;
			duration[0] = (jarak[0] - (0.5*max_velocity*t1))/max_velocity;
			...
			t1 = t1*0.5;
			t2=t1;
			duration[jml_trajectory-1] = (jarak[jml_trajectory-1] - (0.5*max_velocity*t2))/max_velocity;
			cmd_count = 0;
			total_duration = total_duration + t1;
...
void position_trajectory(const ros::TimerEvent& event)
{
	...
	{	
		...
		if(gain_constant < (t1 + duration[0]) && cmd_count == 0)
		{
			if(gain_constant < t1)
			{
...
/*			if(gain_constant < (total_duration + duration[cmd_count]))
	...
	*/		
		...
		if(cmd_count == 12)
		{
			...
			if(gain_constant < total_duration)
			{
			delta_position_reff_x = Vx_cmd[cmd_count] - Vx_cmd[cmd_count]*(exp((10/t2)*(gain_constant-total_duration-(t2/2)))/(exp((10/t2)*(gain_constant-total_duration-(t2/2)))+1));
				delta_position_reff_y = Vy_cmd[cmd_count] - Vy_cmd[cmd_count]*(exp((10/t2)*(gain_constant-total_duration-(t2/2)))/(exp((10/t2)*(gain_constant-total_duration-(t2/2)))+1));
		//		delta_position_reff_x = Vx_cmd[cmd_count] - Vx_cmd[cmd_count]*((gain_constant-total_duration+t2)/t2);
			//	delta_position_reff_y = Vy_cmd[cmd_count] - Vy_cmd[cmd_count]*((gain_constant-total_duration+t2)/t2);	
//				delta_position_reff_x = Vx_cmd[cmd_count];
	//			delta_position_reff_y = Vy_cmd[cmd_count];
			}
			if(gain_constant >= total_duration)
