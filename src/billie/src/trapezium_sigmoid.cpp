...
double velocity_prv_x = 0;
double velocity_prv_y = 0;
...
int main(int argc, char **argv)
{
	...
	while(ros::ok())
	{
		...
		if(velocity_ctrl == 1)
		{
			...
			rate.sleep();
			fprintf(trapezium, "%f, %f, %f, %f, %f, %f, %f, %f \n",position_reff_x, current_position_x, qc_velocity.twist.linear.x, velocity.twist.linear.x,delta_position_reff_x, acceleration, roll, pitch);	
		}
...
void doTakeOff()
{
	...
	trapezium = fopen("trapezium.csv","w");
...
void position_trajectory(const ros::TimerEvent& event)
{
	...
	acceleration = (qc_velocity.twist.linear.x - velocity_prv_x)/dt;
	velocity_prv_x = qc_velocity.twist.linear.x; 
