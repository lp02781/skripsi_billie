...
const double ki = 2.5;
const double kd = 1;
const double kp_yaw = 0.05;
const double ki_yaw = 0.5;
...
void position_trajectory(const ros::TimerEvent& event)
{
	...
	{	
		...
		*/
		...
		{
			position_reff_x = x_awal + radius*cos(((gain_constant/duration)*2*pi) - 0.5*pi);	
			position_reff_y = y_awal + radius*sin(((gain_constant/duration)*2*pi) - 0.5*pi) + radius;	
