...
void position_trajectory(const ros::TimerEvent& event)
{
	...
	{	
		...
		if(gain_constant >= 0 && gain_constant < (duration/2)) 
		{
			position_reff_x = x_awal + (gain_constant/duration)*2*x1_cmd;	
			position_reff_y = y_awal + (gain_constant/duration)*2*y1_cmd;	
			...
			x_awal_baru = qc_position.pose.position.x;
			y_awal_baru = qc_position.pose.position.y;
		}
		if(gain_constant >= (duration/2) && gain_constant < duration) 
		{
			position_reff_x = x_awal_baru + ((gain_constant-duration/2)/duration)*2*x2_cmd;	
			position_reff_y = y_awal_baru + ((gain_constant-duration/2)/duration)*2*y2_cmd;	
			current_yaw_reff = atan2(position_reff_y-position_reff_y_prv,position_reff_x-position_reff_x_prv)/pi*180;		
			position_reff_x_prv = position_reff_x;
			position_reff_y_prv = position_reff_y;
