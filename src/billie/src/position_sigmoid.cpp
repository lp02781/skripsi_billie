...
int main(int argc, char **argv)
{
	...
	while(ros::ok())
	{
		...
		if(velocity_ctrl == 1)
		{
			...
			rate.sleep();
			fprintf(position_data, "%f, %f, %f, %f, %f, %f, %f, %f\n",position_reff, current_position, velocity.twist.linear.x, qc_velocity.twist.linear.x, acceleration, roll, pitch, yaw);
...
void doTakeOff()
{
	...
	position_data = fopen("position.csv","w");
...
void position_trajectory(const ros::TimerEvent& event)
{
	...
	{	
		...
		{
			position_reff = gain_constant;	
		}
		*/
		if(gain_constant >= 0 && gain_constant < duration) 
		{
			position_reff = position_cmd*(exp((10/duration)*(gain_constant-(duration/2)))/(exp((10/duration)*(gain_constant-(duration/2)))+1));	
		}
	...
	if(pid_ctrl == 1){
		...
		current_position = qc_position.pose.position.x;
		current_error = position_reff - current_position;
		...
		ROS_INFO_STREAM("besarnya kecepatan sekarang " << qc_velocity.twist.linear.x);
		ROS_INFO_STREAM("besarnya referensi adalah " << position_reff);
		ROS_INFO_STREAM("besarnya posisi sekarang " << current_position);
